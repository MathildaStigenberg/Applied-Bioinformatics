...
relevant_data <- all_data[ , c("POS", "gt_GT", "REF", "ALT", "Indiv", "gt_AF")] 

relevant_data <- subset(relevant_data, !is.na(gt_GT) & !is.na(gt_AF))

relevant_data$gt_AF <- sprintf("%.4f", as.numeric(as.character(relevant_data$gt_AF)))
...
for (i in 1:len){
  ...
  if (relevant_data$Indiv[i] == sample){
    for (j in relevant_data$POS[i]){
      ...
      if (nchar(relevant_data$REF[relevant_data$ID == i]) > 10 | nchar(relevant_data$ALT[relevant_data$ID == i]) > 10){
      ...
      Frequency <- append(Frequency, relevant_data$gt_AF[relevant_data$ID == i])
      if (!is.null(genes[[j]])){
